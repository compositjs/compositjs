...
const debug = require('debug')('service-composer:service:rest-service');
...
/**
 * Resolving the host value from ENV or context
 * 
 * ``` js
 * ...,
 * "host": {
 *  "source": "environment",
 *  "key": "<environment variable goes here>",
 ...
 * @param {string} param.source Source of param
 * @param {string} param.key Key name of param from source
 * @param {string} param.default Default value of param, if no match found
 * 
 * @returns paramvalue
 */
const resolveServiceParams = (param) => {  
  let value = undefined;  
  
  if(typeof param === "object") {    
    
    switch(param.source) {
      // for param name is sourcing from environment variable
      case "environment":
        value = getValueFromEnv(param.key);
        break;
  ...
  debug("service-param after resolve:", `Parameter is ${param} and value is ${value}`);
...
const resolveServiceURL = (spec, context) => {
  ...
  const host = resolveServiceParams(spec.host);
...
class RestserviceController {
  ...
  _resolveRequestConfigurations(context) {
    ...
    let service = this.spec.service;

    let headerparams = getParamsFromContext(service.headers, context);
    const isJSON = headerparams['content-type'] && headerparams['content-type'].indexOf('json') > -1 ? true : false;
    ...
    if(service.timeout || process.env.DEFAULT_SERVICE_TIMEOUT) {
      config.options.timeout = resolveServiceParams(service.timeout) || process.env.DEFAULT_SERVICE_TIMEOUT;
    }

    // Setting cookies
    let cookieparams = getParamsFromContext(service.cookies, context);
    
    let cookies = [];
    for(let cookieName in cookieparams) {
      cookies.push(cookie.serialize(cookieName, cookieparams[cookieName]));
    }
  ...
  async invoke(context) {
    ...
    const requestConfigurations = this._resolveRequestConfigurations(context);
    let response = {};

    try {   
      
      // Executing service through hystrix
      response = await this._service.execute(requestConfigurations.url, requestConfigurations.options);
      ...
      if(this.spec.service.fallback) {
        ...
        // TODO: should control through flag(ENV or global setting) to change the status code of fallback(ed) responses.
        response.status = 200; 
