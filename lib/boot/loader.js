...
const debug = require('debug')('compositjs:boot:config-loader');
const fs = require('fs');
const utils = require('./util');

const ConfigLoder = exports;
...
function findFiles(dir, name) {
  const master = utils.files.ifExists(`${dir}/${name}`);
  if (!master) {
...
ConfigLoder.loadAppConfig = configRootDir => load(configRootDir, 'config', mergeConfig);

ConfigLoder.loadPlugins = (options = {}) => {
  const plugins = [];
  const pluginFiles = fs.readdirSync(options.dir);
  pluginFiles.filter((file) => {
