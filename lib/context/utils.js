...
const debug = require('debug')('compositjs:context:utils');
const { CONTEXT_PREFIX } = require('../utils/constants');
...
const decor = function (decorators, target, key, desc) {
...
const paramDecorate = function (target, decors) {
  const decorators = decors.map((d, index) => param(index, inject(d)));
  return decor(decorators, target);
};
...
exports.paramDecorate = paramDecorate;
